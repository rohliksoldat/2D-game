# 2D Survival Shooter — local dev via Traefik, deploy to Docker Swarm.
...
export PUBLISH_DOMAIN   ?= 2d.localhost

DEPLOY_HOST             ?= primus.nadoma.net
DEPLOY_SSH_USER         ?= admin
DEPLOY_DIR              ?= /srv/2D-game
DEPLOY_PUBLISH_DOMAIN   ?= 2d.nadoma.net
