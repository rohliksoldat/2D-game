# 2D Survival Shooter — local dev via Traefik, deploy to primus.nadoma.net (Docker Swarm)

STACK_NAME           ?= 2d-game
export PUBLISH_URL   ?= 2d.localhost
export IMAGE_URL     ?= registry.nadoma.net/2d-game
export IMAGE_TAG     ?= $(shell git rev-parse --short HEAD)

PROD_HOST            ?= admin@primus.nadoma.net
PROD_DIR             ?= /srv/2D-game
PROD_PUBLISH_URL     ?= 2d.nadoma.net
